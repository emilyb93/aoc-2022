...
const overlaps = ({ firstStart, firstEnd, secondStart, secondEnd }) => {
  if (firstStart >= secondStart && firstStart <= secondEnd) return true;
  if (firstEnd >= secondStart && firstEnd <= secondEnd) return true;
  if (secondStart >= firstStart && secondStart <= firstEnd) return true;
  if (secondEnd >= firstStart && secondEnd <= firstEnd) return true;
  return false;
};
